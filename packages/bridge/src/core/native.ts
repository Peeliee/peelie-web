...
export function createNativeBridge<S extends BridgeSchema>(
  ...
  return {
    bind(boundHandlers: Handlers<S>): NativeBridge<S> {
      handlers = boundHandlers as unknown as Record<
        string,
